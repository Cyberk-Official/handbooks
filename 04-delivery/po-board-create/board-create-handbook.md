...
# Khởi tạo Board dự án — Cẩm nang cho PO / Tech Lead
...
1. **PO không ngồi cạnh dev cả ngày** — nhưng mở board ra, dưới 10 giây phải biết: tuần này team đang ở đâu, ai đang kẹt, task nào trễ. Không cần hỏi, không cần họp.
...
- **"Review"** — Ai review? Code review hay design review? Chờ review của PO hay client? Mỗi người hiểu khác nhau → task kẹt cột "Review" cả tuần mà không ai biết nó đang chờ ai.
...
❌ Tồi: Task treo ở `Review` 4 ngày. PO hỏi: "Ai đang review?" Không ai biết.
...
Milestone trả lời: "Tuần 34 phải bàn giao cái gì cho khách?" Nếu task không gắn Milestone, PO phải tự nhớ — hoặc mở file khác tra — board không tự trả lời được.
...
Đầu mỗi tuần, PO đổi filter `Week = [tuần mới]` → Sprint Board tự cập nhật. Nhanh, sạch, không cần tạo board mới mỗi sprint.
...
Đây là view **quan trọng nhất**. PO mở Sprint Board ra mỗi sáng — 10 giây biết:
...
Đây là view dành cho **dev, không phải cho PO.** Mỗi sáng dev mở Personal Board — thấy ngay task nào cần làm hôm nay, không cần chờ standup, không cần hỏi Lead.
...
Refactor Board tách riêng nợ kỹ thuật ra, giúp PO và Tech Lead nhìn rõ: còn bao nhiêu nợ, đang tăng hay giảm, có cần dành sprint riêng để trả nợ không.
...
Cần khi: PO muốn chuẩn bị sprint tiếp — kiểm tra tuần sau đã có đủ task chưa, assign chưa, estimate chưa. Không cần ngay khi mới tạo board.
...
Cần khi: Dự án có ≥ 5 Epic chạy song song. PO muốn biết "Feature Payment hoàn thành bao nhiêu phần trăm?" → mở view filter Epic = Payment, đếm Done/Total.
